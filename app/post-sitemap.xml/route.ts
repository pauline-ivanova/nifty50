...
export async function GET(request: NextRequest) {
  ...
  const guidesDirectory = path.join(process.cwd(), 'content', 'guides', 'en');

  const posts = guides.map((guide) => {
    const filePath = path.join(guidesDirectory, `${guide.slug}.mdx`);
    return {
...
<head>
  ...
  <title>Post Sitemap - ${baseUrl.replace('https://', '').replace('http://', '')}</title>
  <style>
...
<body>
  <div class="header">
    <h1>Post Sitemap</h1>
    <p>
      This XML Sitemap contains all posts (guides and articles) on this website.
    </p>
  </div>
  ...
  <div class="container">
    <div class="info">
      This sitemap contains <strong>${posts.length}</strong> posts.
    </div>
    <table>
      <thead>
        <tr>
