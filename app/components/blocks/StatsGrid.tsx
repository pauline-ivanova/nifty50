...
const StatsGrid: React.FC<StatsGridProps> = ({ title, description, stats, padding = 'default', background = 'dark', showPixelBlast = false }) => {
  const paddingClasses = padding === 'top-compact'
    ...
    return (
        <div className={`relative isolate overflow-hidden bg-brand-secondary ${paddingClasses}`}>
            {showPixelBlast && (
                ...
                <div className="mx-auto grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 sm:gap-y-20 lg:mx-0 lg:max-w-none lg:grid-cols-2 items-center">
                    <div className="lg:pr-8 lg:pt-4">
                        <div className="lg:max-w-lg">
                            ...
                            <p className="mt-6 text-lg leading-8 text-brand-silver">{description}</p>
                        </div>
                    </div>
                    <dl className="grid grid-cols-1 gap-8 sm:grid-cols-2">
                        {stats.map((stat) => (
                            <div key={stat.name} className="flex flex-col rounded-lg bg-brand-saffron p-6 text-center hover:bg-brand-saffron-hover transition-colors duration-300">
                                <dd className="text-3xl font-bold leading-8 tracking-tight text-white mb-2">{stat.value}</dd>
