...
export default function TableOfContents({ items, category = 'Basics' }: TableOfContentsProps) {
      ...
      <li key={item.id} className="group">
        <a
          ...
          className={`
            ...
            ${isH2 ? `font-bold ${colors.text}` : 'font-medium text-sm text-gray-600'}
        ...
        >
           ...
           <div className="flex-shrink-0 w-5 flex justify-center items-center">
             <span className={`
               ...
               ${isH2 
                 ? `w-3.5 h-3.5 ring-4 ring-white ${colors.borderAccent} bg-white` 
                 : `w-2.5 h-2.5 border-gray-300 bg-white group-hover:border-gray-400`
               }
             `}>
               {/* Inner dot for H2 */}
               {isH2 && (
    ...
    <div className={`my-8 rounded-2xl border ${colors.border} bg-gradient-to-br ${colors.fromGradient} ${colors.viaGradient} ${colors.toGradient} shadow-lg relative overflow-hidden ${isExpanded ? 'w-full' : 'w-fit'}`}>
      ...
      <div className="relative z-10">
        ...
        <button
          ...
          className="w-full flex items-center justify-between p-5 text-left hover:bg-white/50 transition-colors duration-200 rounded-t-2xl"
        ...
        >
          <nav className="px-5 pb-6" aria-label="Table of contents">
