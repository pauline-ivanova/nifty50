...
export default function ChecklistItemContent({ children }: ChecklistItemContentProps) {
  ...
  // Case 1: Colon is inside a child (e.g. **Title:**)
  // Case 2: Colon is at the start of a text node following a child (e.g. **Title**: Description)
  ...
  for (let i = 0; i < filteredChildren.length; i++) {
    ...
    if (React.isValidElement(child)) {
      ...
      if (childText.includes(':')) {
        // Case 1: Colon inside the element
        // We assume this element is the header (or part of it)
        // We try to split the text inside this element
        ...
        // Check if next sibling starts with colon (Case 2)
  ...
  // If no split found, check if the whole text has a colon and try crude split?
  // Or just render as is. 
  // The user asked for split. If we fail to find the pattern, we might fail to format.
  // But usually the pattern is distinct.
