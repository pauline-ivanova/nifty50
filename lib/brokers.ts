...
const servicesDirectory = path.join(process.cwd(), 'content', 'brokers');
const DEFAULT_LANG = 'en';

export function getSortedBrokersData() {
  const langDirectory = path.join(servicesDirectory, DEFAULT_LANG);
...
export async function getBrokerData(slug: string): Promise<ServiceData> {
  const attemptPath = path.join(servicesDirectory, DEFAULT_LANG, `${slug}.mdx`);
  let fileContents = '';

  if (fs.existsSync(attemptPath)) {
    fileContents = fs.readFileSync(attemptPath, 'utf8');
  } else {
      throw new Error(`Broker data not found for slug: ${slug}`);
  }
